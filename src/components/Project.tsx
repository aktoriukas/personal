...
export default function Project({ project }: Props) {
  ...
  return (
    <div style={{ backgroundColor: project.bgColor }} className={`min-h-screen flex flex-col justify-end`}>
      ...
      <div className="about-project px-8 pb-10 md:px-24 md:pb-32 max-w-4xl relative">
        <div className="flex items-center">
          ...
          <h2 className={`text-3xl md:text-5xl slide-left ${dark ? "text-offwhite" : ""}`}>{title}</h2>
        </div>

        <span style={{ height: "1px" }} className="w-24 md:w-36 bg-offwhite mb-2 ml-1 block slide-right" />

        <div className="stack flex flex-wrap slide-left ml-1">
          {stack.map((item) => (
            <span key={item} className={`text-xs md:text-sm pr-2 ${dark ? "text-offwhite" : "text-charcle"}`}>
              {item}
            </span>
          ))}
        </div>
        {link && (
          <div className={`pt-4 slide-left ${dark ? "text-white" : "text-charcle"} text-opacity-60`}>
